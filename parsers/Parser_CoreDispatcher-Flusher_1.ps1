    ...
    foreach($reader in $readers)
{
            ...
            if($reader_Core.BaseStream.Name -eq $reader.BaseStream.Name) 
        {
            ...
            if($table.Count -le 2000 -and $counter -gt 0 ){$counter=18000
            $string_builder = New-Object System.Text.StringBuilder
            [array]$a=@()}
    ...
    while (!($reader.EndOfStream))
    {
            ...
            if($counter -eq 20000){
                    ...
                    [array]$a+=($string_builder.ToString().split([Environment]::NewLine, [StringSplitOptions]::RemoveEmptyEntries) -match "TRAN_ID" )
                    write-host "reading done" ($string_builder.ToString().split([Environment]::NewLine, [StringSplitOptions]::RemoveEmptyEntries) -match "TRAN_ID").count
                    Remove-Variable string_builder -Force
                    [System.GC]::Collect()
                    ...
                    foreach($sb in $a) {
                        ...
                        $time = $sb.split(";")[0]
                        $TRAN_ID =$sb.Split(",")[0].Split(":")[-1].trim()
                        $table[$TRAN_ID] = $time
