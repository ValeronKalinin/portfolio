...
$result=@{}
            ...
            $basesE ='zSQLupdate',
                    ...
                    Invoke-Command -ComputerName "10.50.130.207" -Credential $mycred -ScriptBlock {
            ...
            foreach($baseE in $basesE)
            {
                    ...
                    BACKUP DATABASE [$baseE] TO  DISK = N'E:\bak\1611_shrink\$baseE.bak' 
                    WITH NOFORMAT, NOINIT,  NAME = N'$baseE-Full Database Backup', SKIP, NOREWIND, NOUNLOAD,COMPRESSION,  STATS = 10
                    GO"
                    
                     start-job -Name $baseE -ScriptBlock  { param ($script) Invoke-SqlCmd -ServerInstance "10.50.130.207" -Database 'master' -Username sa -Password 12345678 -Query "$script" -QueryTimeout 36000 -Verbose 4>&1} -ArgumentList $script
